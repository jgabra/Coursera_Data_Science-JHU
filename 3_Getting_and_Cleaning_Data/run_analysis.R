...
names(combined_data_subset)[4:length(combined_data_subset)]<-varNames2[4:dim(varNames2)[1],8]#Rename the columns names of the data set excluding the first three which are already appropriately named
...
melted_combined_data_subset<-melt(combined_data_subset,id=c("Subject_Number","Origin_DataSet","Activity")) #"Melts" the data base to only have factors/levels for subject number, the origin data set (if wanted), and the activity performed. All the measured variable labels will then be listed in the 4th column with their respective values in the 5th column


narrow_combined_data_subset<-melted_combined_data_subset[,c(1,3,4,5)] #Excludes the "Origin_DataSet" factor on whether the data was test/train. No subject is in both and it is not needed per the specific request of the project prompt step 5
...
wide_combined_data_subset$Origin_DataSet<-NULL #Gets rid of origin_dataSet as in narrow (per prompt)
