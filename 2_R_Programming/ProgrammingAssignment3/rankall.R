...
rankall <- function(outcome,num="best") {
        ## This will use the rankhospital function to find the rankings for all 
        ## states
        ## 
        outcomeData <- read.csv("outcome-of-care-measures.csv",colClasses =
              "character")
        St <- unique(outcomeData[,7])
        St<-St[order(St)]
        
        
        output<-data.frame("hospital"=NA,"state"=St)
        
        for(i in 1:length(St)){
                output[i,1]<-rankhospital(St[i],outcome,num)
        }
        output
        # ## Read outcome data
        # outcomeData <- read.csv("outcome-of-care-measures.csv",colClasses = 
        ...
        # 
        ## Return a data fram with the hospital names and the abbreviated state
        ## names
        ...
        # ## death rate
        # print(as.character(subData$Hospital.Name[num]))
